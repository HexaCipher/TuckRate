...
- Schema lives in `supabase/schemas/*.sql` (declarative) and is applied automatically by the Supabase GitHub integration on push to the connected branch — a push can mutate the live DB. `supabase/seed.sql` is NOT auto-applied; run it once via the Supabase SQL editor.
